...
这些是按场景加载的协作指引，不会在安装时自动启动 Agent。MCP 的 10 个工具负责读取和写入结构化记录（包括重点关注）；公共引用文件定义权限、幂等重试、版本冲突及质量证据规则。下载时生成 `references/record-schema.json`，始终与当前服务器契约一致，在线 `workhub://schema` 优先。
