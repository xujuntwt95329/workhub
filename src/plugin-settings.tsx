...
export function PluginSettings({
  ...
  useEffect(() => {
    let active = true;
    setLoadError("");
    api<PluginMetadata>("/api/plugins/claude")
      .then((value) => {
        if (active) {
          setMetadata(value);
          setBaseUrl(value.publicUrl);
        }
      })
      .catch((error) => {
  ...
  const parsed = pluginDownloadSchema.safeParse({
    target,
    authentication,
    baseUrl,
  });
    ...
    <div className="plugin-settings">
      <section className="panel plugin-hero">
        ...
        <div className="grow">
          <div className="plugin-kicker">
            WORKHUB × CLAUDE <span>v{metadata.version}</span>
          </div>
          <h2>让 Agent 跟上你的工作方式。</h2>
          <p>把上下文、协作规则和工程工具，一起带进 Claude。</p>
        </div>
      ...
      <section className="panel plugin-install">
        ...
        <ol className="plugin-steps">
          <li>
            <div>
              <strong>{target === "code" ? "解压并安装" : "上传并启用"}</strong>
              {target === "code" ? (
                <>
                  <p>
                    完整解压到固定目录，在 workhub-marketplace
                    的上一级目录执行：
                  </p>
                  <div className="plugin-command">
                    <pre>
                      <code>{codeInstallCommands}</code>
                    </pre>
                    <button
                      className="icon-button"
                      aria-label="复制安装命令"
                      onClick={() => void copy(codeInstallCommands)}
                    >
                      <Copy size={16} />
                    </button>
                  </div>
          ...
          <li>
            <div>
              ...
              {authentication === "oauth" ? (
                ...
                <>
                  <p>创建凭证后，在启动 Claude 的终端安全输入 Token。</p>
                  ...
                  <div className="plugin-command">
                    <pre>
                      <code>
                        {shell === "powershell" ? tokenPowerShell : tokenBash}
                      </code>
                    </pre>
                    <button
                      className="icon-button"
          ...
          <li>
            <div>
              ...
              <div className="plugin-example">
                <code>/workhub:connect TASK-001</code>
        ...
        <details className="plugin-faq">
          ...
          <p>
            安装包内 README 包含完整步骤。Claude Code 更新：替换原解压目录，执行{" "}
            <code>claude plugin marketplace update workhub-local</code> 和{" "}
            <code>claude plugin update workhub@workhub-local --scope user</code>
            ，然后重启；桌面端重新上传插件。
          </p>
          <p>
            OAuth 面向整个工作空间；需要隔离任务时，使用 Claude Code Token
            版。Agent 不具备最终审批、拒绝、删除、豁免和验收权限。
          </p>
          <p>
